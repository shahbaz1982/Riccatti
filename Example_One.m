...
n = 20; % You can modify n as needed

% Create the vector e
e = ones(n, 1);
% Create the tridiagonal matrix D
main_diag = -0.6 * ones(n, 1);
sub_diag = -1.25 * ones(n-1, 1);
super_diag = -0.5 * ones(n-1, 1);
D = diag(main_diag) + diag(sub_diag, -1) + diag(super_diag, 1);
% Adjust D by subtracting 0.5(e, e, ..., e)
D = D - 0.5 * (e * e');
